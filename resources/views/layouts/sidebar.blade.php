<nav class="transition-all duration-300 -left-[600px] sm:left-0 w-3/4 sm:w-64 h-screen bg-glass fixed z-10 rounded-r-3xl border-r border-white/10" :class="{
        ...
        <div>
            <div class="flex flex-col space-y-3 mb-5">
                <x-nav-link :href="route('dashboard')" :active="request()->routeIs('dashboard')">
                    <x-slot name="icon"><i class="fas fa-tachometer-alt text-violet-400"></i></x-slot>
                    <x-slot name="name">仪表盘</x-slot>
                </x-nav-link>
            </div>
            <div class="flex flex-col space-y-2 mb-5">
                <p class="text-gray-500 text-sm mx-4">我的</p>
                <x-nav-link :href="route('upload')" :active="request()->routeIs('upload')">
                    <x-slot name="icon"><i class="fas fa-cloud-upload-alt text-violet-400"></i></x-slot>
                    <x-slot name="name">上传图片</x-slot>
                </x-nav-link>
                <x-nav-link :href="route('images')" :active="request()->routeIs('images')">
                    <x-slot name="icon"><i class="fas fa-images text-violet-400"></i></x-slot>
                    <x-slot name="name">我的图片</x-slot>
                </x-nav-link>
                <x-nav-link :href="route('settings')" :active="request()->routeIs('settings')">
                    <x-slot name="icon"><i class="fas fa-user-cog text-violet-400"></i></x-slot>
            ...
            <div class="flex flex-col space-y-2 mb-5">
                <p class="text-gray-500 text-sm mx-4">公共</p>
                @if(\App\Utils::config(\App\Enums\ConfigKey::IsEnableGallery))
                <x-nav-link :href="route('gallery')" :active="request()->routeIs('gallery')">
                    <x-slot name="icon"><i class="fas fa-chalkboard text-violet-400"></i></x-slot>
                ...
                <x-nav-link :href="route('api')" :active="request()->routeIs('api')">
                    <x-slot name="icon"><i class="fas fa-link text-violet-400"></i></x-slot>
            ...
            <div class="flex flex-col space-y-2 mb-5">
                ...
                <x-nav-link :href="route('admin.strategies')" :active="request()->is('admin/strategies*')">
                    <x-slot name="icon"><i class="fas fa-hdd text-violet-400"></i></x-slot>
                    <x-slot name="name">储存策略</x-slot>
                </x-nav-link>
                <x-nav-link :href="route('admin.settings')" :active="request()->is('admin/settings*')">
                    <x-slot name="icon"><i class="fas fa-cogs text-violet-400"></i></x-slot>
